...
/* ---------------- Staff photo library ----------------
   Every portrait ever uploaded for staff, with who it belongs to — a media library for
   the Edit Staff form.

   WHY IT EXISTS
   Cloudinary names an upload with a random id, so when the link on the staff record
   was lost (23 of 51 portraits on production went that way) the picture survived but
   nothing said whose face it was. Here an unattached portrait can be recognised by eye
   and attached with one click, instead of being photographed again.

   WHO SEES IT
   The listing needs EDIT on the staff module (server/photos.js), the same right as
   uploading — it shows every portrait in the register, not only the rows a scoped
   account may see. Accounts without staff access never receive portraits at all: the
   roster they are sent is filtered server-side (access.filterStaff) before it leaves. */
function StaffPhotoLibrary({ f, set, store, empId, editing, onClose }){
  const [rows,setRows]=React.useState(null);
  const [counts,setCounts]=React.useState(null);
  const [err,setErr]=React.useState('');
  const [filter,setFilter]=React.useState('unattached');   // 'unattached' | 'all'
  const [busy,setBusy]=React.useState('');
  const me=String(f.emp_id||'');
  const myId=f.id!=null?String(f.id):'';

  React.useEffect(()=>{ let live=true;
    fetch('/api/upload/library',{credentials:'same-origin'}).then(r=>r.json()).then(j=>{
      if(!live) return;
      if(!j||!j.ok){ setErr((j&&j.error)||'Could not load the library.'); setRows([]); return; }
      setRows(j.assets||[]); setCounts(j.counts||null);
    }).catch(()=>{ if(live){ setErr('Could not reach the server.'); setRows([]); } });
    return ()=>{live=false;};
  },[]);
  ...
  const pick=async(a)=>{
    // Taking a portrait that belongs to someone else is a deliberate act, never a slip.
    if(a.attachedTo&&!isMine(a)){
      ...
      const ok=(window.UI&&window.UI.confirm)?await window.UI.confirm({title:'Photo belongs to someone else',message:q,confirmLabel:'Use it here',cancelLabel:'Keep'}):window.confirm(q);
      if(!ok) return;
    }
    const photo={url:a.url,publicId:a.publicId,updatedAt:Date.now()};
    setBusy(a.publicId); setErr('');
    try{
      if(set) set('photo',photo);
      if(editing&&store&&store.update&&empId!=null){ try{ store.update(empId,{photo}); }catch(e){} }
      // Existing record: attach on the server too, so the link outlives this browser.
      if(editing&&(f.id!=null||f.emp_id)){
        const r=await fetch('/api/upload/attach',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',
          body:JSON.stringify({publicId:a.publicId,url:a.url,staffId:f.id!=null?f.id:null,empId:f.emp_id||null,staffName:f.name||''})});
        const j=await r.json().catch(()=>({ok:false}));
        if(!r.ok||!j.ok){ setErr(j.error||'Attached on this device, but the server copy could not be written — press Save changes.'); }
      }
      onClose();
    }catch(e){ setErr('Could not attach the photo.'); }
    finally{ setBusy(''); }
  };
  ...
  const modal = (
    <div onMouseDown={(ev)=>{ if(ev.target===ev.currentTarget) onClose(); }}
      ...
      <div style={{background:'#fff',width:'min(920px,100%)',maxHeight:'88vh',borderRadius:14,boxShadow:'0 24px 60px rgba(5,12,24,.4)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'14px 18px',borderBottom:'1px solid var(--line-2)',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
          <div>
            <div style={{fontSize:15.5,fontWeight:800,color:'var(--ink)'}}>Uploaded staff photos</div>
            <div style={{fontSize:11.5,color:'var(--muted)',marginTop:2}}>
              {counts?<>{counts.total} in the library · <b style={{color:counts.unattached?'#b5670a':'#157a43'}}>{counts.unattached} not attached to anyone</b></>:'Loading…'}
              {f.name?<> · choosing for <b style={{color:'var(--ink-2)'}}>{f.name}</b></>:null}
            </div>
          </div>
          <span style={{flex:1}}/>
          <div className="seg" style={{display:'flex',gap:4}}>
            {[['unattached','Not attached'],['all','All photos']].map(([v,l])=>(
              <button key={v} type="button" onClick={()=>setFilter(v)}
                style={{border:'1px solid '+(filter===v?'var(--blue)':'var(--line)'),background:filter===v?'var(--blue-50)':'#fff',color:filter===v?'var(--blue-700)':'var(--ink-2)',padding:'5px 11px',borderRadius:7,fontSize:11.5,fontWeight:700,cursor:'pointer'}}>{l}</button>
            ))}
          </div>
          <button type="button" className="icon-btn" onClick={onClose} title="Close (Esc)"><Ic d={I.x} s={15}/></button>
        </div>

        {err?<div style={{margin:'12px 18px 0',padding:'9px 12px',borderRadius:8,background:'#fdf3f4',border:'1px solid #f0c2ca',color:'#a32c41',fontSize:12.5,fontWeight:600}}>{err}</div>:null}

        <div style={{padding:16,overflowY:'auto'}}>
          {rows===null?<div style={{color:'var(--muted)',fontSize:12.5,padding:'30px 0',textAlign:'center'}}>Loading the library…</div>
          :shown.length===0?<div style={{color:'var(--faint)',fontSize:12.5,padding:'30px 0',textAlign:'center'}}>{filter==='unattached'?'Every uploaded photo is attached to someone.':'No photos have been uploaded yet.'}</div>
          :(
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:12}}>
              {shown.map(a=>{ const mine=isMine(a), taken=a.attachedTo&&!mine; return (
                <button key={a.publicId} type="button" onClick={()=>pick(a)} disabled={!!busy} title={taken?'Attached to '+a.attachedTo.name:mine?'Already this staff member’s photo':'Attach to '+(f.name||'this record')}
                  style={{border:'1px solid '+(mine?'#1f9d57':taken?'var(--line)':'#e6c98a'),borderRadius:11,padding:0,background:'#fff',cursor:'pointer',textAlign:'left',overflow:'hidden',
                    boxShadow:mine?'0 0 0 2px #1f9d5733':'none',opacity:busy&&busy!==a.publicId?.6:1}}>
                  <img src={a.thumbUrl||a.url} alt="" style={{width:'100%',aspectRatio:'1',objectFit:'cover',display:'block',background:'var(--panel-2)'}}/>
                  <div style={{padding:'7px 9px 9px'}}>
                    {mine
                      ? <div style={{fontSize:10.5,fontWeight:800,color:'#157a43'}}>✓ Current photo</div>
                      : taken
                        ? <div style={{fontSize:10.5,fontWeight:700,color:'var(--ink-2)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>→ {a.attachedTo.name||a.attachedTo.empId}</div>
                        : <div style={{fontSize:10.5,fontWeight:800,color:'#b5670a'}}>Not attached</div>}
                    <div style={{fontSize:10,color:'var(--muted)',marginTop:2}}>{when(a.createdAt)}{a.bytes?' · '+Math.round(a.bytes/1024)+' KB':''}</div>
                  </div>
                </button>
              );})}
            </div>
